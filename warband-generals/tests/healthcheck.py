#!/usr/bin/env python3
"""warband-generals 靜態健檢。無遊戲環境下抓 XML/交叉引用/相依/鐵律錯誤（仿 Mod 1 風格）。"""
import re
import sys
...
ROOT = Path(__file__).resolve().parent.parent          # warband-generals/
...
if about is not None:
    r = about.getroot()
    check(r.findtext("packageId") == "pas.officers.warband", "About packageId 應為 pas.officers.warband")
...
if roles:
    check(roles[0].findtext("defName") == "pas_warband_General",
          "角色 defName 應為 pas_warband_General")
...
for f, tree in parsed.items():
    for node in tree.getroot().iter():
        cls = node.get("Class")
        if cls and cls.startswith("pas.officers.warband."):
            check(re.search(rf"class\s+{cls.split('.')[-1]}\b", src) is not None,
                  f"XML 引用的類 {cls} 不存在於 Source/")

# 5. C# 引用的 pas_warband_* key/defName 都在 XML（Translate/DefDatabase 防呆）
xml_all = "\n".join(f.read_text(encoding="utf-8", errors="ignore") for f in xml_files)
for ref in set(re.findall(r"pas_warband_\w+", src)):
...
for name in ("CreateWarband", "CreateWarObjectOfType", "ResolveCombat_Units", "GetInspectString"):
