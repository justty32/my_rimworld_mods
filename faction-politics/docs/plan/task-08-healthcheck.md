# Task 8: 靜態健檢 tests/healthcheck.py

**Files:**
- Create: `faction-politics/tests/healthcheck.py`

8 檢，沿 npc-outposts 健檢的教訓：類存在性用**結構化掃描**（Class= 屬性 + def 節點 tag）防 packageId 誤報；defName/key 防呆要把 **Languages Keyed key 一起算**。

- [ ] **Step 1: tests/healthcheck.py**
...
Expected: `healthcheck OK`（任何 FAIL 先判斷是實作錯還是健檢誤報——npc-outposts 兩次教訓都是健檢要修，但先查實作）

- [ ] **Step 3: Commit**

```powershell
git -C C:\code\mine\my_rimworld_mods add faction-politics/tests
git -C C:\code\mine\my_rimworld_mods commit -m @'
test: faction-politics 靜態健檢（XML/交叉引用/軟相容不變式/翻譯一致）
